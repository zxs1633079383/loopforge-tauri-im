...
│  定时 data-has-schedule · 回复链 data-reply-id                │
├──────────────────────────────────────────────────────────────┤
│ composer: input + 交互件                                       │
...
| `data-has-schedule` | channel.has_schedule_post | 1.10 |
...
| `data-has-schedule` | （频道级·非消息行·见 §1.2） | `emit_schedule_created`/`canceled` hasSchedulePost | 1.10 | 频道行属性 |
...
| 1.10 定时消息 | CP+CL | `schedule-btn`/`cancel-schedule-btn`+`onSchedule()`/`onCancelSchedule()`；频道行 `data-has-schedule` | ⬜ |
...
| **2 发消息** | 1.1✅/1.2✅/1.9/1.8/1.10/1.3🌙 | ML 已有；**MessageRow 扩** `urgent`/`reactions`；CP 加 `send-urgent-btn`/行级 `quick-reply-btn`/`schedule-btn`；CL 频道行加 `data-has-schedule`。组件方法 `onSendUrgent`/`onQuickReply`/`onSchedule`。 |
