...
describe('UC-5.6w · 公告写族 save/read/delete（WS post_update echo·① 可达绿·②④ ⛔backend-down）', () => {
  let CHANNEL_ID;
  let TARGET_POST_ID; // read/delete 的既有公告 postId 目标（必须是真公告 server_id）
  ...
  before(async () => {
    ...
    // read/delete 目标须作用于**真公告**（#3 实证：read/delete 对不存在/非公告 postId → 后端拒）。
    // 故 before 先建一条真公告（type:'ANNOUNCEMENT'·后端 validateAnnouncementPost 要求该类型·
    // type:'TEXT' 会被拒），捕获返回 announcementId 作 read/delete 的 TARGET_POST_ID。
    // 退化顺序：真 announcementId → 首个已渲染公告 server_id。拿不到真实公告 id 则 fail-loud。
    await invokeBridge('set_uc', { uc: '__quiescence__' });
    const created = await invokeBridge('im_announcement_save', {
      channelId: CHANNEL_ID,
      type: 'ANNOUNCEMENT',
      message: `announcement-seed-${Math.random().toString(36).slice(2, 8)}`,
    });
    const res = created?.result ?? {};
    const createdId =
      res.announcementId ?? res.data?.announcementId ?? res.id ?? res.postId ?? null;
    if (createdId) {
      TARGET_POST_ID = createdId;
    } else {
      TARGET_POST_ID = await getFirstAnnouncementServerId();
    }
    expect(TARGET_POST_ID).toBeTruthy();
    console.log(`[UC-5.6w before] CHANNEL_ID=${CHANNEL_ID} TARGET_POST_ID=${TARGET_POST_ID}（真公告=${!!createdId}）`);
  });
  ...
  it('②④ save：post_update echo → im:post:updated（fat 13 键）+ message batch_update（⛔backend-down）', async () => {
    ...
    await waitPostUpdateEcho({ ch: ECHO_CH }, 'save', ['im:post:received', 'im:post:updated']);
    await invokeBridge('set_uc', { uc: '__quiescence__' });
