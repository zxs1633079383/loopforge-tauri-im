...
// ── P0b 6 面 oracle 扩展：⓪ IpcIn≡Inbound 纯壳不变量 + WsRecv 断言化 ──────────────
//
// 真源：docs/orchestration/6-facet-oracle.md。当前四面 oracle 纯输出侧；本节验输入侧两个新面：
//   ⓪ 纯壳不变量（C013 量化）：IpcIn.args ≡ Inbound.args（归一后逐字段相等·壳零中间 shaping）
//   WsRecv 断言化：入站 go echo 帧从只观测升级为可断言（vs expect.wsRecv）
// 每个绿配可证伪对偶（C008/HX-C011：破坏即红·断点定位正确）。

console.log('· P0b ⓪ 纯壳不变量 IpcIn≡Inbound（C013 量化）');
{
  const mkIpc = (args, command = 'im_send') =>
    JSON.stringify({ run_id: 'r', uc_id: 'UC-ps', facet: 'ipc-in', hop: 'ipc-in', seq: 1,
      payload: { command, args } });
  const mkInb = (args, command = 'im_send_message') =>
    JSON.stringify({ run_id: 'r', uc_id: 'UC-ps', facet: 'inbound', hop: 'inbound', seq: 2,
      payload: { command, args } });

  // 金标：壳透传（IpcIn camel·Inbound snake·归一后逐字段相等）→ 纯壳绿。
  const psGood = mkIpc({ channelId: 'c1', text: 'hi', temporaryId: 't9' }) + '\n' +
    mkInb({ channel_id: 'c1', text: 'hi', temporary_id: 't9' });
  const expPs = { ucId: 'UC-ps', pureShell: { injectedKeys: [] } };
  ...
  const rMutate = checkPureShellInvariant({ jsonl: psMutate, expect: expPs, ucId: 'UC-ps' });
  eq(rMutate.ok, false, '可证伪：Inbound 篡改 text 值（壳加工）→ 纯壳红');
  ok(rMutate.issues.some((i) => i.includes('text')), '可证伪：纯壳红指出篡改字段 text');

  // 可证伪 b（丢字段）：Inbound 丢失 temporaryId → 壳吞 user arg → 纯壳红。
  const psDrop = mkIpc({ channelId: 'c1', text: 'hi', temporaryId: 't9' }) + '\n' +
    mkInb({ channel_id: 'c1', text: 'hi' });
  const rDrop = checkPureShellInvariant({ jsonl: psDrop, expect: expPs, ucId: 'UC-ps' });
  eq(rDrop.ok, false, '可证伪：Inbound 丢失 temporaryId（壳吞 arg）→ 纯壳红');

  // 可证伪 c（未授权注入）：Inbound 多 type 且未登记 injectedKeys → 壳擅自 shaping → 纯壳红。
  const psInject = mkIpc({ channelId: 'c1', text: 'hi', temporaryId: 't9' }) + '\n' +
    mkInb({ channel_id: 'c1', text: 'hi', temporary_id: 't9', type: 'TEXT' });
  const rInject = checkPureShellInvariant({ jsonl: psInject, expect: expPs, ucId: 'UC-ps' });
  eq(rInject.ok, false, '可证伪：Inbound 注入未授权 type（壳 shaping）→ 纯壳红');

  // 授权注入：injectedKeys 登记 type（身份/默认注入·人审授权）→ 同一束放行 → 纯壳绿。
  const expPsInj = { ucId: 'UC-ps', pureShell: { injectedKeys: ['type'] } };
  const rInjOk = checkPureShellInvariant({ jsonl: psInject, expect: expPsInj, ucId: 'UC-ps' });
  eq(rInjOk.ok, true, '授权注入：injectedKeys=[type] 登记后注入 type 放行 → 纯壳绿');

  // 可证伪 d（缺埋点）：无 Inbound tee → 壳透传纯度不可证 → 纯壳红（非静默放过）。
  const psNoInb = mkIpc({ channelId: 'c1', text: 'hi', temporaryId: 't9' });
  const rNoInb = checkPureShellInvariant({ jsonl: psNoInb, expect: expPs, ucId: 'UC-ps' });
  eq(rNoInb.ok, false, '可证伪：缺 Inbound 埋点 → 纯壳红（透传纯度不可证·非放水）');

  // N/A：未声明 pureShell（旧 UC 叠加面）→ 不裁定（optional·总绿·不破坏既有四面）。
  const rNa = checkPureShellInvariant({ jsonl: psGood, expect: { ucId: 'UC-ps' }, ucId: 'UC-ps' });
  eq(rNa.ok, true, 'N/A：未声明 pureShell → optional 不裁定（旧 UC 叠加面零破坏）');
}

console.log('· P0b WsRecv 断言化（入站 go echo 帧可断言）');
{
  const mkWs = (data, action = 'post') =>
    JSON.stringify({ run_id: 'r', uc_id: 'UC-ws', facet: 'ws-recv', hop: 'ws-recv', seq: 1,
      payload: { action, data } });

  const wsExpect = { ucId: 'UC-ws', wsRecv: { action: 'post', dataFields: { channelId: '*', id: '*' } } };

  // 金标：入站 post 帧 data 含 channelId + id → WsRecv 绿。
  const wsGood = mkWs({ channelId: 'c1', id: 's7', temporaryId: 't9', eventSeq: 40114 });
  const { events: evGood } = parseJsonl(wsGood);
  const rWsGood = diffWsRecv(wsExpect, evGood, 'UC-ws');
  eq(rWsGood.ok, true, 'WsRecv 绿：入站 post 帧 data 含 channelId + id');

  // 可证伪 a（帧缺失）：窗口内无 action=post 入站帧 → WsRecv 红（断在 go echo→recv 这跳）。
  const wsWrongAction = mkWs({ channelId: 'c1', id: 's7' }, 'increment_channel');
  const { events: evWrong } = parseJsonl(wsWrongAction);
  const rWsWrong = diffWsRecv(wsExpect, evWrong, 'UC-ws');
  eq(rWsWrong.ok, false, '可证伪：无 action=post 入站帧 → WsRecv 红');

  // 可证伪 b（字段缺）：入站 post 帧 data 缺 id → WsRecv 红（dataFields 刚性）。
  const wsNoId = mkWs({ channelId: 'c1', temporaryId: 't9' });
  const { events: evNoId } = parseJsonl(wsNoId);
  const rWsNoId = diffWsRecv(wsExpect, evNoId, 'UC-ws');
  eq(rWsNoId.ok, false, '可证伪：入站 post 帧 data 缺 id → WsRecv 红');

  // N/A：未声明 wsRecv（旧 UC 叠加面）→ optional 不裁定。
  const rWsNa = diffWsRecv({ ucId: 'UC-ws' }, evGood, 'UC-ws');
  eq(rWsNa.ok, true, 'N/A：未声明 wsRecv → optional 不裁定（旧 UC 叠加面零破坏）');
}

console.log('· P0b 六面合并入口 runSixFacet（四面 + ⓪纯壳 + WsRecv）');
{
  // 复用 UC-send-1 金标束（四面金标）+ 叠加 ipc-in/inbound + wsRecv/pureShell 期望。
  // 注：不改冻结 EXPECT 文件·在内存 clone 上叠加新面（contract-readonly：旧四面契约源不动）。
  const sixExpect = {
    ...EXPECT,
    wsRecv: { action: 'post', dataFields: { channelId: '*', id: '*' } },
    pureShell: { injectedKeys: [] },
  };
  const ipcLine = JSON.stringify({ run_id: 'r-sample', uc_id: 'UC-send-1', facet: 'ipc-in', hop: 'ipc-in', seq: 5,
    payload: { command: 'im_send', args: { channelId: 'e9wnenyb', text: 'hi', temporaryId: 't-abc123' } } });
  const inbLine = JSON.stringify({ run_id: 'r-sample', uc_id: 'UC-send-1', facet: 'inbound', hop: 'inbound', seq: 6,
    payload: { command: 'im_send_message', args: { channel_id: 'e9wnenyb', text: 'hi', temporary_id: 't-abc123' } } });
  const sixJsonl = ipcLine + '\n' + inbLine + '\n' + SAMPLE;

  const rep6 = runSixFacet({ jsonl: sixJsonl, expect: sixExpect, dom: DOM_GOOD, ucId: 'UC-send-1' });
  eq(rep6.green, true, '六面全绿：四面金标 + 纯壳 IpcIn≡Inbound + WsRecv post 帧断言');
  ok(rep6.facets['pure-shell'].ok, '六面绿：pure-shell 面绿');
  ok(rep6.facets['ws-recv'].ok, '六面绿：ws-recv 面绿');

  // 可证伪（纯壳）：Inbound 篡改 text → 六面 brokenAt=pure-shell（最上游输入侧面先断）。
  const inbBad = JSON.stringify({ run_id: 'r-sample', uc_id: 'UC-send-1', facet: 'inbound', hop: 'inbound', seq: 6,
    payload: { command: 'im_send_message', args: { channel_id: 'e9wnenyb', text: 'HACKED', temporary_id: 't-abc123' } } });
  const sixBadPs = ipcLine + '\n' + inbBad + '\n' + SAMPLE;
  const rep6BadPs = runSixFacet({ jsonl: sixBadPs, expect: sixExpect, dom: DOM_GOOD, ucId: 'UC-send-1' });
  eq(rep6BadPs.green, false, '可证伪：Inbound 篡改 text → 六面非绿');
  eq(rep6BadPs.brokenAt, 'pure-shell', '可证伪：六面 brokenAt=pure-shell（输入侧面最上游先断）');

  // 可证伪（WsRecv）：去掉入站 post 帧（换 SAMPLE 的 post 帧 action）→ brokenAt=ws-recv。
  const sampleNoPost = SAMPLE.split('\n')
    .map((l) => (l.includes('"action":"post"') ? l.replace('"action":"post"', '"action":"noop"') : l))
    .join('\n');
  const sixNoWs = ipcLine + '\n' + inbLine + '\n' + sampleNoPost;
  const rep6NoWs = runSixFacet({ jsonl: sixNoWs, expect: sixExpect, dom: DOM_GOOD, ucId: 'UC-send-1' });
  eq(rep6NoWs.facets['ws-recv'].ok, false, '可证伪：无 action=post 入站帧 → ws-recv 面红');
}

console.log('· P0b corr-key .args 探针（IpcIn/Inbound payload 领域键抽取）');
{
  // IpcIn/Inbound payload 形态 {command, args:{...}}：领域键嵌在 .args（非顶层/.data/.body）→
  // 须探 .args 使 ⓪ 两面经 ch/tmp 与发送束聚同 corr_key（与装饰器 event.rs::extract_corr_key 同步）。
  const dIpc = extractDims({ command: 'im_send', args: { channelId: 'c1', temporaryId: 't9' } });
  eq(dIpc.ch, 'c1', '.args 探针：IpcIn payload 抽出 ch');
  eq(dIpc.tmp, 't9', '.args 探针：IpcIn payload 抽出 tmp');
  const dInb = extractDims({ command: 'im_send_message', args: { channel_id: 'c1', temporary_id: 't9' } });
  eq(dInb.ch, 'c1', '.args 探针：Inbound payload snake 抽出 ch');
  eq(dInb.tmp, 't9', '.args 探针：Inbound payload snake 抽出 tmp');
}
