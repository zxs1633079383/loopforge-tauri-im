...
## 3. Go-only safe rerun

当前真实链路收口默认只跑 `go-only` profile：
...
Token 只能从进程环境变量 `APIFOX_TOKEN` 读取。不要把真实 token 写进脚本、文档、命令示例、终端输出或任何 git-tracked 文件；dry-run 也只能输出不含 token 的命令说明。

`go-only` profile 明确排除 Java 场景：

- `UC-8.x 投票 CRUD`
- `UC-8.x 平均分 CRUD`

Apifox go-only 全绿只能报告为 **HTTP preflight green**。它不代表 WS、DOM、reducer、L1 或 L2 已通过，后续仍需 WebdriverIO / reducer 证据承接。
