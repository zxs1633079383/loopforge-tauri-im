---
name: "source-command-loop-engine"
description: "启动 loop-Engine 真 autonomous run(确定性内核 + 真 agent 数据面·自驱 UC 到四面绿)"
---

# source-command-loop-engine

Use this skill when the user asks to run the migrated source command `loop-engine`.
...
收到本命令时,用 **Workflow 工具**调用已存的脚本(仓库约定 `.Codex/workflows/`):

```
Workflow({
  scriptPath: ".Codex/workflows/loop-engine.js",
