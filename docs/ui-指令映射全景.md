...
|---|---|---|---|---|
| `revoke-btn` | （占位·"UC-1.5 接通"·命令已通 UI 待接） | （待核·im_revoke） | POST `/api/cses/posts/revoke` → WS `posts_update`/`post_update` | 1.5 |
...
| `locate-btn` | `locatePost` | （读族纯本地·无 invoke·debugLocatePost 桥） | （复用 query_result ②④ + ③ 高亮） | 2.3 |
...
| `cancel-schedule-btn` | （占位·"UC-1.10 接通"） | （待核·im_cancel_schedule） | POST `/api/cses/posts/cancelSchedule` → WS `post_schedule_canceled` | 1.10 |
...
> **6.2 结构性例外**：add/remove manger 后端 WS 已注释·L1 单账号 ② emit_channel_member_updated 不到达 → 壳出站成功后**乐观刷 `data-admin`**（唯一壳乐观例外·权威态由 L2 #45 广播帧对账）。
...
| POST `/posts/cancelSchedule` | 1.10 | 屏③ `cancel-schedule-btn`（占位·待接通） | ✅ |
...
| POST `/posts/revoke` | 1.5 | 屏② `revoke-btn`（占位·命令已通 UI 待接） | ✅ |
...
| 12 | `channel_member_role_updated` | `emit_channel_member_updated`/`emit_channel_update` | （L1 graceful no-op·data-admin 乐观刷·权威 L2#45） | 屏⑤ MB | 6.2 | ✅ |
...
| 维度 | 数量 | 说明 |
|---|---|---|
| ✅ 有壳投影渲染入口（apply* 消费·全挂 UC） | 17 | #1-18（除 #19 bot 变体）·post_schedule_canceled(#7) 投影工厂存在但壳 onBus **无 schedule-canceled 分支**（待核·UC-1.10 取消定时占位未接通） |
| 🚫 无入口（bot/agent 变体） | 1 | #19 `post`(agent/bot) |
| ❓ 漏网 | 0 | quit_company→11.2·post_pin→5.5b 已覆盖 |

> **WS 全部 18 个业务 action 有投影工厂**·其中 17 有壳 apply* 渲染回灌路径。**待核 1 项**：`post_schedule_canceled`（#7）有 `emit_schedule_canceled` 投影工厂，但壳 `onBus` 未见对应 channel 分支（UC-1.10 `cancel-schedule-btn` 当前为占位 no-op）·建议 UC-1.10 接通时补壳分支。
...
> **占位/待核 invoke**（UI 有 btn 但 store 未接 invoke·占位 no-op）：`onRevoke`（UC-1.5·"命令已通 UI 待接"·待核 im_revoke）· `onCancelSchedule`（UC-1.10·待核 im_cancel_schedule）。CLAUDE C007 要求模板 `(click)` 必配组件方法（已满足·方法体为占位），但 store invoke 未接通——属 UC rollout 待铺项·非缺陷。

---

# 诚实结论

1. **壳=纯渲染铁律（C013）成立**：每条入站 = 用户动作→`invoke im_*`（args 透传·body/identity/wire 构造全在 helix-im commands.rs + outbound/*.rs）；每条渲染 = `im:* 投影 → apply* → data-* 1:1 绑定`（解析/对账/role→admin/四源合并/wire 归一全已下沉 helix render-ready 通道：`im:channel:members`/`im:channel:replies`/`im:todo:updated`/message_item_data render-ready 字段）。唯一壳乐观例外 = UC-6.2 `data-admin`（后端 WS 注释·L1 ② 不到达·权威 L2#45）。

2. **UI 覆盖完整性**：7 屏覆盖**全部可达业务 UC**（30+ UC）；HTTP 有 UI 入口 72/124（✅70+🟡2·其余 52 = bot/服务端/运维/废弃/归属 java）；WS 17/18 业务 action 有壳渲染路径（待核 1：post_schedule_canceled）；入站 invoke 47 个实接 + 2 占位待接（revoke / cancelSchedule）。

3. **待核项（3）**：① `post_schedule_canceled` 壳 onBus 无分支（UC-1.10 取消定时占位）② `onRevoke` UI 占位未接 im_revoke（UC-1.5 撤回·命令已通）③ 书签列表 body 渲染路径——`applyReadResult` 当前仅处理 health/byIds drop·书签/公告/在线/模块/查频道 read body 的前端「从 body 抽渲染」逻辑未在 store 落地（投影回灌已到·data-bookmark-id/卡片渲染待补·属 UC rollout 待铺）。
