...
describe('UC-10.2 · 系统通知（系统 NOTICE post·② 投影 ③ DOM ④ 落库·① N/A）', () => {
  ...
  it('②③④：改群名派生 channelUpdate 系统 NOTICE post → im:post:received(type=NOTICE) + DOM data-system-notice + message 落库（① N/A）', async () => {
    ...
    // —— ②：先等 change/displayName 的 channelUpdate 系统 NOTICE post 投影到达（props.type=channelUpdate·
    //    content=新名）·窗口必须保持 UC-10.2 开启直到它到（rename echo 慢于建群 join post·若靠 DOM
    //    data-system-notice 早停会被 join post(也是 NOTICE→systemNotice) 提前满足→窗口提前关→
    //    channelUpdate post 落 __quiescence__·reducer uc 过滤漏掉·守可证伪：echo 不到则超时红）——
    await browser.waitUntil(
      async () => {
        ...
        for (const ln of lines) {
          ...
          if (
            ...
            o?.payload?.data?.props?.type === 'channelUpdate' &&
            o?.payload?.data?.props?.content === NEW_NAME
          )
            return true;
        }
        return false;
      },
      {
        ...
        interval: 200,
        timeoutMsg:
          'channelUpdate 系统 NOTICE post 投影未到达 UC-10.2 窗口（断在 change/displayName→WS post(channelUpdate)→im:post:received）',
      }
    );
