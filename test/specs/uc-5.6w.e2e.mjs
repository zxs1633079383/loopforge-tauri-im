...
describe('UC-5.6w · 公告写族 save/read/delete（WS post_update echo·① 可达绿·②④ ⛔backend-down）', () => {
  ...
  before(async () => {
    ...
    // read/delete 目标须作用于**真公告 post**（#3 实证：read/delete 对不存在/非公告 postId → 后端拒）。
    // 故 before 先建一条真公告（type:'ANNOUNCEMENT'），再从真实投影取 server msg_id 作 postId。
    // Tauri 命令返回 void；Go HTTP 响应里的 announcementId 不是 read/delete 所需的 postId。
    await invokeBridge('set_uc', { uc: '__quiescence__' });
    TARGET_POST_ID =
      (await createAnnouncementPost(CHANNEL_ID, 'announcement-seed')) ??
      (await getFirstAnnouncementServerId());
    expect(TARGET_POST_ID).toBeTruthy();
    console.log(`[UC-5.6w before] CHANNEL_ID=${CHANNEL_ID} TARGET_POST_ID=${TARGET_POST_ID}（真实公告 post）`);
  });
  ...
  it('②④ save：post_update echo → im:post:updated（fat 13 键）+ message batch_update（⛔backend-down）', async () => {
    ...
    await waitAnnouncementPostId(ECHO_CH, message);
    await invokeBridge('set_uc', { uc: '__quiescence__' });
