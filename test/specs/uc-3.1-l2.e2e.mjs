// UC-3.1 L2 · 会话已读·双账号 e2e（issue #15 / tracker #47·channels/view 触发 post_read echo）—— WebdriverIO，直连 4445。
//
...
// 以 B=678 身份 POST channels/view 标整会话已读（会话已读·child_process 跑 l2-act.sh·cookieId 桥）。
function readAsB(channelId) {
  const out = execFileSync('bash', [L2_ACT, 'read', channelId], {
...
describe('UC-3.1 L2 · 会话已读双账号（#15 / #47）', () => {
  let CHANNEL_ID;
  let MSG_ID;
  ...
  it('②④③：A 发消息 → B=678 标已读 → A 收 post_read echo → im:post:read 投影 + read_bits 落库 + DOM', async () => {
    ...
    await invokeBridge('set_uc', { uc: 'UC-3.1-L2-send' });
    ...
    console.log(`[UC-3.1-L2 send] A=444 消息 server_id=${MSG_ID}`);

    // —— 第二步：开 UC 窗口 → B=678 以自身身份 channels/view 标整会话已读 ——
    await invokeBridge('set_uc', { uc: 'UC-3.1' });
    const readResp = readAsB(CHANNEL_ID);
    console.log(`[UC-3.1-L2 B-read] 678 channels/view → ${readResp.trim()}`);
    // ① 在 B 侧：channels/view HTTP 成功（CommonRes status=SUCCESS）。守可证伪：B read 失败 → 无 echo → ②④ 红。
    ...
    console.log('[UC-3.1-L2 echo] A=444 收到 im:post:read（post_read echo·msg_id 命中）');
    ...
    const domEvidenceFile = await captureDomEvidence(browser, 'uc-3.1-l2-read-observer', [
    ...
    console.log('[UC-3.1-L2 四面报告] ' + report.summary);
